...
<body>
	...
	<form action="${pageContext.request.contextPath }/admin/selectUserField.action"
		method="post">
		请输入关键字： <input type="text" name="key"> 条件满足： <select
			name="field">
			<option value="username">学生姓名</option>
			<option value="sort">所学专业</option>
			<option value="teacher">指导教师</option>
			<option value="sxtype">实习类型</option>
	...
	显示数据库表
	<br>
	满足查询：<font color=red>${a }</font>，关键字：<font color=red>${key }</font> 的条件如下：<br>
			<table border="1">
				<tr bgcolor="aabbcc">
					<th>序号</th>
					<th>姓名</th>
